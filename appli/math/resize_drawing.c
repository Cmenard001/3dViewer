...
void resize_drawing(drawing_two_dims_t *drawing, dist_t width)
{
    ...
    {
        dist_t current_dist = dist_from_origin(drawing->segment[segment_index].p1);
        if (current_dist > max_dist)
        {
            max_dist = current_dist;
        }
        current_dist = dist_from_origin(drawing->segment[segment_index].p2);
        if (current_dist > max_dist)
        {
            max_dist = current_dist;
        }
    ...
    // et on translate le dessin pour qu'il soit centré
    for(int segment_index = 0; segment_index < drawing->nb_segment; segment_index++)
    {
        drawing->segment[segment_index].p1.x += width / 2;
        drawing->segment[segment_index].p1.y += width / 2;
        drawing->segment[segment_index].p2.x += width / 2;
        drawing->segment[segment_index].p2.y += width / 2;
    }
}
